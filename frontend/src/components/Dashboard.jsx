...
const Dashboard = () => {
  ...
  const isSettingsValid =
    visibility !== "" && deadline !== "" && deadline !== null;
  ...
  useEffect(() => {
    ...
    if (savedJobData) {
      try {
        ...
        // Use application_deadline if available, fallback to end_date for backward compatibility
        setDeadline(
          parsedData.application_deadline || parsedData.end_date || "",
        );
  ...
  const handleProcessResumes = async () => {
    ...
    try {
      ...
      if (!apiBaseUrl) {
        console.warn(
          "VITE_BASE_URL is not defined in environment variables. Falling back to relative path.",
        );
      }
      ...
      console.log(
        "Uploading resumes to:",
        `${apiBaseUrl}/api/resume/upload-resumes`,
      );
  ...
  const handleCreateNewJob = () => {
    ...
    console.log("Posting job on the career portal");
  ...
  const handlePostJob = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/drive/${drive_id}/publish`,
        {
  ...
  const handlePostJobToCareerPortal = async () => {
    ...
    try {
      const jobPayload = {
        ...
        hiringType:
          jobData.experience_type === "experienced" ? "Experienced" : "Fresher",
        experienceLevel:
          jobData.experience_type === "experienced" ? "Experienced" : "Fresher",
        jobType: jobData.job_type === "internship" ? "Internship" : "Full-time",
        startDate:
          jobData.start_date && !isNaN(new Date(jobData.start_date).getTime())
            ? new Date(jobData.start_date).toISOString()
            : new Date().toISOString(),
        endDate:
          jobData.end_date && !isNaN(new Date(jobData.end_date).getTime())
      ...
      toast.error(
        error.response?.data?.message || "Failed to post job to HiKareers",
      );
    } finally {
      setIsPosting(true);
  ...
  return (
    <div className="flex-1 w-full pb-10">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm mb-6">
          ...
          <div className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm border-b pb-5 border-gray-100">
              <div>
                <span className="text-gray-500 block text-xs font-semibold uppercase tracking-wide mb-1">
                  Job ID
                </span>
              ...
              {jobData.coding_questions?.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-orange-600">
                    {jobData.coding_questions.length}
                  </span>
                </div>
              )}
            </div>
      ...
      {jobData && jobData.postJobOnPortal && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm mb-6">
          ...
          <div className="p-4">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full appearance-none bg-white border border-gray-200 text-gray-700 py-2.5 px-4 pr-8 rounded-lg leading-tight focus:outline-none focus:border-indigo-500"
                  >
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white border border-gray-200 text-gray-700 py-2 px-4 rounded-lg focus:outline-none focus:border-indigo-500"
                  />
              ...
              <div className="flex gap-3">
                {postingConfig.isPosted && visibility === "public" && (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 py-2.5 px-4 bg-gray-100 border border-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-200 transition-colors"
                    title="Copy Link"
                  >
                    <Share2 size={18} />
                  </button>
            ...
            <div className="flex items-start gap-2 text-xs text-gray-500">
              {!deadline ? (
                <span>
                  * Please set an application deadline to enable posting.
                </span>
              ) : (
                <span className="flex items-center gap-1">
                  <CheckCircle size={12} /> Ready to publish. Candidates will be
                  able to apply until {deadline}.
                </span>
              )}
            </div>
          </div>
      ...
      {showLinkModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all animate-in fade-in zoom-in duration-300"
          ...
          >
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 p-6 text-white text-center relative">
              ...
              <p className="text-indigo-100 mt-1">
                Your job is now live on the Career Portal.
              </p>
            </div>
